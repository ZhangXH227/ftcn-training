...
class I3D8x8(nn.Module):
    def __init__(self) -> None:
        ...
        if my_cfg.model.inco.i3d_routine:
            self.cfg = cfg
        # self.FAD_Head = FAD_Head(size=224)
    ...
    def forward(
        ...
        # fre_inputs = []
        # for input in inputs:
        #     input_FAD = self.FAD_Head(input)
        #     input_new = torch.cat((input, input_FAD), dim=1)
        #     fre_inputs.append(input_new)
        # pred = self.resnet(fre_inputs)[:, 0]
        pred = self.resnet(inputs)[:, 0]
        return pred
